    ...
    ep_store.update(&mut cx, |store, _cx| {
        ...
        if let PredictionProvider::Zeta2(format) = provider {
            if format != ZetaFormat::default() {
                let model_id = std::env::var("MIZEN_ZETA_MODEL").ok();
                let environment = std::env::var("MIZEN_ZETA_ENVIRONMENT").ok();
...
pub async fn predict_baseten(
    ...
    let model_id =
        std::env::var("MIZEN_ZETA_MODEL").context("MIZEN_ZETA_MODEL environment variable required")?;
