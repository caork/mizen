...
fn git_sha() -> Option<String> {
    if let Ok(sha) = std::env::var("MIZEN_COMMIT_SHA") {
        return Some(sha);
    }
...
pub fn compile(manifest: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (icon_filename, product_name) = match channel {
        "stable" => ("app-icon.ico", "Mizen"),
        "preview" => ("app-icon-preview.ico", "Mizen Preview"),
        "nightly" => ("app-icon-nightly.ico", "Mizen Nightly"),
        _ => ("app-icon-dev.ico", "Mizen Dev"),
    };
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUE "CompanyName", "Mizen Contributors\0"
            VALUE "LegalCopyright", "Copyright 2025 Mizen Contributors\0"
        END
    END
    BLOCK "VarFileInfo"
    BEGIN
...
"#
    ...
    let rc_path = out_dir.join("mizen_resources.rc");
    std::fs::write(&rc_path, rc_content)?;

    if let Ok(toolkit_path) = std::env::var("MIZEN_RC_TOOLKIT_PATH") {
