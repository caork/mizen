...
fn main() {
    ...
    let git_sha = match std::env::var("MIZEN_COMMIT_SHA").ok() {
        Some(git_sha) => {
    ...
    if let Some(git_sha) = git_sha {
        println!("cargo:rustc-env=MIZEN_COMMIT_SHA={git_sha}");

        if let Some(build_identifier) = option_env!("GITHUB_RUN_NUMBER") {
            println!("cargo:rustc-env=MIZEN_BUILD_ID={build_identifier}");
        }
            ...
            && build_profile == "release"
        {
            ...
            println!("cargo::warning=Info: using '{git_sha}' hash for MIZEN_COMMIT_SHA env var");
        }
    }
